...
export class CsFormField extends Vue {
  ...
  public dateFormatted = this.formatDate(
  ...
  public get selectedObject(): any | null {
    if (
      ...
      if (o) {
        return o.find(
          (s) => s[this.field!.keyValue!] == this.target![this.field!._key!]
        );
      }
    }
    return null;
  ...
  public parseDate(date) {
    ...
    if (typeof date === 'number') {
      ...
      return `${nd.getFullYear}-${nd
        .getMonth()
        .toString()
        .padStart(2, '0')}-${nd.getDate().toString().padStart(2, '0')}`;
    } else {
  ...
  public deleteFromArray(key: string, index: number) {
    if (key !== undefined && this.field && this.target !== undefined) {
  ...
  public addObject(field: IFormFieldOptions) {
    if (!this.target || !field._key || !field.keyValuesType) {
      return;
    }
  ...
  public keyObjectChange(
    ...
    this.target[field._key] = renameProp(
      oldValue,
      newValue,
      this.target[field._key]
    );
