      ...
      switch (this.type) {
        case 'geojson':
          ...
          axios
            ...
            .then(async (response) => {
              if (response) {
                this._data = this.InitFeatures(
                  response.data as DataSet,
                  this.id
                );
                this._loaded = true;
                await this.initFeatureTypes(featureTypes);
      ...
      if (this._meta && Object.keys(this._meta).length > 0) {
        ...
        Object.keys(this._meta).forEach((k) => {
          if (this._meta && this._meta.hasOwnProperty(k)) {
            ...
            this._meta[k] = plainToClass(FeatureType, this._meta[k]);

            this.updateFeatureTypePropertyMap(this._meta[k]);
          }
        });
        this._meta = plainToClass(FeatureTypes, this._meta) as FeatureTypes;
        // this.updateMeta();
      } else {
