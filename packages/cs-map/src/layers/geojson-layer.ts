...
export class GeojsonLayer implements IMapLayer, IMapLayerType {
	...
	public get opacity(): number | undefined {
		if (this._initialized && this.id && this._manager && this._manager.MapControl && this.type) {
			try {
				let maxOpacity = 0;
				for (const prop of this.layerTypeProps()) {
					const op = this._manager.MapControl.getPaintProperty(this.id, this.type + '-opacity') * 100;
					if (op > maxOpacity) {
						maxOpacity = op;
					}
				}
	...
	setOpacity(value: number) {
		this._opacity = value;
		if (this._initialized && this.id && this._manager && this._manager.MapControl) {
			for (const prop of this.layerTypeProps()) {
				this._manager.MapControl.setPaintProperty(this.id, prop + '-opacity', value / 100.0);
	...
	public initLayer(manager: MapLayers) {
		...
		let l = typeof this.getBounds === 'function' ? this : plainToClass(GeojsonLayer, this);
		...
		if (typeof l.source === 'string') {
			if (manager._sources && manager._sources.layers.hasOwnProperty(l.source)) {
	...
	private registerLayerExtensions(l: IMapLayer) {
		if (l.extensions) {
				...
				const extensionType = CsMap.layerExtensions.find(le => le.id === ext.id);
			...
			try {
				...
				if (diff !== undefined) {
					...
					if (this._manager && this._manager.MapControl) {
						this._manager.updateLayerSource(this, this._source._geojson);
					}
				}
			} catch (e) {
