...
import { GraphElement, GraphDatasource, BaseElementProperties, FeatureType } from '@csnext/cs-data';
  ...
  public async all(options?: IAllOptions): Promise<any[]> {
    return new Promise(async (resolve, reject) => {
      ...
      for (const element of list) {
        let skip = false;
        if (options.skipclasses && options.skipclasses.length > 0) {
            if (FeatureType.isType(element, options.skipclasses[0])) {
                skip = true;
            }          
        }
        if (!skip) {
