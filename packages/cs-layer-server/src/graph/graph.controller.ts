import { Req, Request, Get, Controller, Query, Inject, Render, Param, Post, Body, Logger, forwardRef, Optional } from '@nestjs/common';
...
export class GraphController {
  ...
  constructor(
    @Optional() @Inject(forwardRef(() => 'DB')) public readonly graph: GraphService,
    @Inject(forwardRef(() => LayerService)) private layerService: LayerService,
    @Inject(forwardRef(() => FilesService)) private filesService: FilesService,
    @Inject(forwardRef(() => OfflineService)) private offlineService: OfflineService,
    @Inject(forwardRef(() => DefaultWebSocketGateway)) private readonly socket: DefaultWebSocketGateway
  ) {}
  ...
  checkOfflineImages(): Promise<any> {
    return new Promise(async (resolve, reject) => {
      // resolve('done'); return;
      if (!this.graph?.source || !this.offlineService) {
        reject();
        return;
      }
  ...
  public search(@Query('query') query: string): Promise<SearchResult | undefined> {
    return new Promise((resolve, reject) => {
      ...
      const res = this.graph.source.fuse
        .search(query)
        .filter((s) => s.score && s.score < 0.2)
        .slice(0, 10) as SearchResult;

      let f = JSON.parse(
        JSON.stringify(res, (key, value: GraphElement) => {
          // if (value.properties)
          // {
  ...
  link(@Body() body: any, @Req() request: Request, @Query('fromId') fromId: string, @Query('toId') toId: string, @Query('classId') classId: string) {
    return new Promise(async (resolve, reject) => {
      ...
      let l = new Link();
      try {
        await this.graph.source.addEdge({
          fromId: fromId,
          toId: toId,
          classId: classId,
          properties: body,
      ...
      this.graph.db
        .link(Link.fromParam(fromId, toId, classId, body))
        .then((r) => {
          resolve(r);
        })
        .catch((e) => {
  ...
  unlink(@Query('id') id: string) {
    return new Promise(async (resolve, reject) => {
      if (!this.graph.db?.unlinkId || !this.graph.source) {
        return { result: 'error' };
      }
      try {
  ...
  store(
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const res = await this.graph.store(body, type, id, source);
        resolve(GraphElement.getFlat(res));
