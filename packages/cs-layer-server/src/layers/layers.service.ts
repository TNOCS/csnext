...
import { ISourcePluginType, ISourcePlugin, ServerConfig, LayerDefinition, LayerSource, Helpers } from '../classes';
...
export class LayerService extends AggregateRoot {
  ...
  public static AddSourcePlugin(type: ISourcePluginType) {
    if (LayerService.sourcePlugins.findIndex((et) => et.id === type.id) === -1) {
  ...
  public async init(configFile?: string, serverPath?: string) {
    if (configFile) {
      ...
      this.absoluteConfigPath = path.join(serverPath ? serverPath : '', configFile);
      ...
      if (fs.existsSync(this.absoluteConfigPath)) {
        ...
        if (configString) {
          ...
          // }
          Logger.log(`Config file loaded: ${this.absoluteConfigPath}`, 'layer-server');
        } else {
          Logger.log(`Error opening config file: ${this.absoluteConfigPath}`, 'layer-server');
  ...
  public importLayers() {
    if (this.config.importPath) {
      const absoluteImportPath = path.join(this.config.serverPath || '', this.config.importPath);
      // make sure it exists
      ...
      const absoluteImportCompletedPath = path.join(absoluteImportPath, 'completed');
      ...
      Logger.log(`Import path specified, loading sources from: ${absoluteImportPath}`, 'layer-server');
      fs.readdirSync(absoluteImportPath).forEach(async (file) => {
        const absoluteImportFile = path.join(absoluteImportPath, file);
        const absoluteCompletedFile = path.join(absoluteImportCompletedPath, file);
        ...
        if (sourcePlugin && typeof sourcePlugin.import === 'function') {
          sourcePlugin
            ...
            .then((sourceImport) => {
              ...
              const absoluteFilePath = path.join(this.config.serverPath || '', this.config.sourcePath || '', id);
              // make sure it exists
              if (!fs.existsSync(absoluteFilePath)) {
                fs.mkdirSync(absoluteFilePath);
              }

              const absoluteSourcePath = path.join(absoluteFilePath, f);
              fs.writeFileSync(absoluteSourcePath, JSON.stringify(sourceImport));
              Logger.log(`Import succeeded, saved to: ${absoluteSourcePath}`, 'layer-server');

              // move original file
              fs.renameSync(absoluteImportFile, absoluteCompletedFile);
  ...
  public reloadTypes(complete = false): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this.init(this.configFile, this.serverPath)
        .then((r) => {
          resolve(true);
        })
        .catch((e) => {
  ...
  public saveServerConfig() {
    ...
    const duplicate = JSON.parse(JSON.stringify(this.config)) as ServerConfig;
  ...
  public initLayer(layer: LayerDefinition): Promise<LayerDefinition> {
    return new Promise<LayerDefinition>(async (resolve) => {
      ...
      // set sourceUrl
      if (layer.source) {
        layer.sourceUrl = this.config.serverUrl + '/sources/' + layer.id;
      }
      if (!layer.version) {
        layer.version = '0.0.1';
      }
  ...
  public queueSocketUpdate(source: LayerSource, feature: Feature) {
    if (source) {
      if (!this.socketSources.hasOwnProperty(source.id)) {
        ...
        if (source._socketQueue && Object.keys(source._socketQueue).length > 0) {
          this.lock.acquire(source.id, () => {
            if (this.socket && this.socket.server) {
              ...
              this.socket.server.emit(address, source._socketQueue);
  ...
  public sendLayerUpdate(id: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this.getLayerSourceById(id)
        .then((s) => {
          if (this.socket && this.socket.server) {
            this.socket.server.emit('layer/' + id, JSON.stringify(s));
          }
          resolve(true);
        })
        .catch((e) => {
  ...
  public updateFeature(sourceid: string, feature: Feature, featureId?: string): Promise<Feature | undefined> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (source && source.features) {
          ...
          const existingFeatureIndex = source.features.findIndex((f) => f.id === feature.id);
          // not found, add
  ...
  public putLayerDefinitionById(id: string, body: LayerDefinition): Promise<LayerDefinition> {
    return new Promise(async (resolve, reject) => {
      ...
      if (defIndex >= 0) {
        ...
        try {
          ...
          if (plugin && typeof plugin.createEmpty === 'function') {
            try {
              const newSource = await plugin.createEmpty(path.join(this.config.serverPath, this.config.sourcePath), newLayerDef);
        ...
        .then((def) => {
          ...
          if (def) {
            ...
            if (def.externalUrl !== undefined) {
              ...
              if (def.externalCacheDuration > 0 && def._layerSource) {
                  ...
                  .then(async (response) => {
                    if (response.data) {
                      if (def.externalCacheDuration > 0) {
                        def._layerSource = response.data;
                        try {
                          // create layer meta data
                          const meta = await new GeojsonSource().createMeta(def._layerSource);
                          if (meta && meta.featureTypes) {
                            def.featureTypes = meta.featureTypes;
                          }
                        } catch (e) {
  ...
  public loadLayerSource(def: LayerDefinition, options?: QueryOptions): Promise<LayerSource | undefined> {
    return new Promise(async (resolve, reject) => {
      ...
      if (plugin && typeof plugin.query === 'function') {
        ...
        if (this.config.connections && def.connectionId && this.config.connections.hasOwnProperty(def.connectionId)) {
          if (def && def.query) {
            plugin
              .query(this.config.connections[def.connectionId], def.query, options)
              .then((r) => {
                resolve(r.source);
                return;
              })
        ...
        try {
          ...
          const loadResult = await plugin.loadSource(def, this.config);
        ...
        try {
          const fileName = path.join(this.config.serverPath || '', this.config.sourcePath || '', def.id, def.source);

          if (def.meta) {
            this.updateMetaFilePath(def);
          }
          const loadResult = await plugin.load(fileName, def._localMeta);
      ...
      this.getLayerSourceById(id)
        .then((s) => {
          if (s) {
            ...
            if (this.socket && this.socket.server) {
              this.socket.server.emit('layer/' + id, JSON.stringify(body));
            }
            this.saveSource(s);
          }
        })
  ...
  private getExtensionSourcePlugin(ext: string): ISourcePlugin | undefined {
    const plugin = LayerService.sourcePlugins.find((p) => p.fileExtensions !== undefined && p.fileExtensions.indexOf(ext.toLowerCase()) !== -1);
  ...
  private getSourcePlugin(type: string): ISourcePlugin | undefined {
    ...
    const plugin = LayerService.sourcePlugins.find((p) => p.source !== undefined && p.source === type);
    if (plugin && typeof plugin.getInstance === 'function') {
      return plugin.getInstance();
    }
  }
