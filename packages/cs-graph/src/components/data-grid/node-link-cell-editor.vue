...
import { GraphDatasource, GraphElement, PropertyType, PropertyValueType, RelationType } from '@csnext/cs-data';
...
export default class NodeLinkCellEditor extends Vue {
    ...
    public updatedValue(value: GraphElement) {        
        if (!this.graph || !value?.id || !this.element || !this.relation?.type) {return; }    
        this.graph
        .addNewEdge({
